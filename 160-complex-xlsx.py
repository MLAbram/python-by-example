...
try:
        ...
        with open(file_path, 'r') as f:
            ...
            for idx, val in enumerate(header_list):
                if re.search('_num', val):
                    ws_lnd_data.append([file_name, lnd_db_name,
                                        file_name_wo_ext, val,
                                        ' numeric(13,2)'])
                    ws_raw_data.append([file_name, raw_db_name,
                                        file_name_wo_ext, val,
                                        ' numeric(13,2)'])
                    ...
                    ws_lnd_data.append([file_name, lnd_db_name,
                                        ...
                                        ' int'])
                    ws_raw_data.append([file_name, raw_db_name,
                                        file_name_wo_ext, val, ' int'])
                elif re.search('_dt', val):
                    ws_lnd_data.append([file_name, lnd_db_name,
                                        file_name_wo_ext, val, ' date'])
                    ws_raw_data.append([file_name, raw_db_name,
                                        file_name_wo_ext, val, ' date'])
                else:
                    ws_lnd_data.append([file_name, lnd_db_name,
                                        file_name_wo_ext, val, ' text'])
                    ws_raw_data.append([file_name, raw_db_name,
                                        file_name_wo_ext, val, ' text'])
    ...
    f_path = os.path.expanduser('~/.logs/python-xlsx-by-example.log')
